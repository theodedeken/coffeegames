...
public final class Resources {
    ...
    public void load(String resourceFile) {
        Map<String, ResourceNamespace> test = Loader.loadResourceMap(
            resourceFile
        );
        for (Entry<String, ResourceNamespace> entry : test.entrySet()) {
            resourceMap.putAll(
                Loader.loadResources(entry.getKey(), entry.getValue())
            );
        }
    }
    ...
    public <T extends Resource> Map<String, T> getNamespaceResources(
        String namespace,
        Class<T> resourceClass
    ) {
        ...
        for (Entry<String, Resource> resource : resourceMap.entrySet()) {
            if (resource.getKey().startsWith(namespace + ".")) {
                output.put(
                    resource.getKey(),
                    resourceClass.cast(resource.getValue())
                );
            }
        }
        return output;
