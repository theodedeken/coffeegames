...
public final class MatrixMaker {
     ...
     */
    public static MatrixD getTransformationMatrix(
        VectorD coord,
        double rot,
        VectorD scale
    ) {
        return getRotationationMatrix(rot)
            .multiplicate(getScaleMatrix(scale))
     ...
     */
    public static MatrixD orthographic(
        double left,
        double right,
        double bottom,
        double top,
        ...
        return new MatrixD(
            ...
            PROJECTION_DIM,
            new double[] {
                PROJECTION_NUMERATOR / (right - left),
                0,
                0,
                -(right + left) / (right - left),
                0,
                PROJECTION_NUMERATOR / (top - bottom),
                0,
                -(top + bottom) / (top - bottom),
                0,
                0,
                -PROJECTION_NUMERATOR / (far - near),
                -(far + near) / (far - near),
                0,
                0,
                0,
