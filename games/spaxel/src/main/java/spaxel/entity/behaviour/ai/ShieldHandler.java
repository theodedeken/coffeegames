...
public abstract class ShieldHandler extends AIHandler {
    ...
    public void execute(Entity entity) {
        ...
        TransformationStorage trnsStore = (TransformationStorage) parent.getComponent(
            SpaxelComponent.TRANSFORMATION
        );
        Set<Entity> projectiles = entity
            .getStream()
            .getEntities(SpaxelComponent.HIT);
        for (Entity projectile : projectiles) {
            TransformationStorage prTrnsStore = (TransformationStorage) projectile.getComponent(
                SpaxelComponent.TRANSFORMATION
            );
            ...
            if (
                ...
                trnsStore
                    .getPosition()
                    .sum(prTrnsStore.getPosition().multiplicate(-1))
                    .length() <
                SHIELD_RADIUS
            ) {
