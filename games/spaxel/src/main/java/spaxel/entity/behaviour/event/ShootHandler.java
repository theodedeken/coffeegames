...
public class ShootHandler extends EventHandler {
    ...
    public void activate(Entity entity) {
        CooldownStorage cdStore = (CooldownStorage) entity.getComponent(
            SpaxelComponent.COOLDOWN
        );
        if (cdStore.getCurrentCooldown() == 0) {
            Entity parent = entity.getParent();
            TransformationStorage pc = (TransformationStorage) parent.getComponent(
                SpaxelComponent.TRANSFORMATION
            );
            StackStorage sc = (StackStorage) entity.getComponent(
                SpaxelComponent.STACK
            );
            ShootStorage shtStore = (ShootStorage) entity.getComponent(
                SpaxelComponent.SHOOT
            );
            ...
            for (int i = 0; i <= (sc.getStacks() - 1); i++) {
                Entity projectile = pri.produce(
                    (TransformationStorage) pc.copy(),
                    parent
                );
                ...
                double dx =
                    Math.sin(pc.getRotation() + offset) * pmc.getSpeed();
                double dy =
                    Math.cos(pc.getRotation() + offset) * pmc.getSpeed();
