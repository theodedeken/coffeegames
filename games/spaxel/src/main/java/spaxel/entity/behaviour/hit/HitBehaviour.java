...
public class HitBehaviour extends Behaviour {
    ...
    public void execute(Entity entity) {
        Set<Entity> colliders = entity
            .getStream()
            .getEntities(SpaxelComponent.HITSHAPE);
        // Entity parent = entity.getParent();
        HitshapeStorage cc = (HitshapeStorage) entity.getComponent(
            SpaxelComponent.HITSHAPE
        );
        TransformationStorage pc = (TransformationStorage) entity.getComponent(
            SpaxelComponent.TRANSFORMATION
        );
        MatrixD eTransform = MatrixMaker.getTransRotationMatrix(
            pc.getPosition(),
            pc.getRotation()
        );
        ...
        for (Entity collider : colliders) {
            // if (collider != parent) {
            HitshapeStorage ccc = (HitshapeStorage) collider.getComponent(
                SpaxelComponent.HITSHAPE
            );
            TransformationStorage cpc = (TransformationStorage) collider.getComponent(
                SpaxelComponent.TRANSFORMATION
            );
            MatrixD cTransform = MatrixMaker.getTransRotationMatrix(
                cpc.getPosition(),
                cpc.getRotation()
            );
