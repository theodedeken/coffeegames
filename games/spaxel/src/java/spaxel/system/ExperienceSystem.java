...
public class ExperienceSystem extends GameSystem {
    ...
    public void update() {
        Set<Entity> entities = Engine.get().getNEntityStream().getEntities(SpaxelComponent.EXPERIENCE);
        for (Entity entity : entities) {
            ExperienceStorage ec = (ExperienceStorage) entity.getComponent(SpaxelComponent.EXPERIENCE);
            if (ec.ding()) {
                ec.levelUp();
                HealthStorage hc = (HealthStorage) entity.getComponent(SpaxelComponent.HEALTH);
