        ...
        if (statStore.canMove()) {
            ...
            if (keyboard.get(SpaxelKey.LEFT).isDown()) {
                velChange =
                    new VectorD(
                        Math.sin(
                            trnsStore.getRotation() - Constants.HALF_CIRLCE
                        ),
                        Math.cos(
                            trnsStore.getRotation() - Constants.HALF_CIRLCE
                        )
            ...
            if (
                chngStore.getPositionChange().sum(velChange).length() <
                mvStore.getSpeed()
            ) {
                chngStore.setPositionChange(
                    chngStore.getPositionChange().sum(velChange)
                );
            } else {
                chngStore.setPositionChange(
                    chngStore
                        ...
                        .sum(
                            chngStore
                                ...
                                .multiplicate(
                                    -1 /
                                    (mvStore.getSpeed() * Constants.SPEED_MULT)
                                )
                        )
                );
            }
