...
public class MarkerHandler extends AIHandler {
    ...
    public void execute(Entity entity) {
        ...
        TransformationStorage parentPos = (TransformationStorage) entity
            .getParent()
            .getComponent(SpaxelComponent.TRANSFORMATION);
        RenderStorage mrc = (RenderStorage) entity.getComponent(
            SpaxelComponent.RENDER_STORE
        );
        VectorD renderPos = parentPos
            .getPosition()
            .sum(Engine.get().getGameState().getScreenOffset());
        if (
            renderPos.getValue(0) < -MARKER_THRESHOLD ||
            ...
            VectorD intersect = getIntersection(
                ...
                new VectorD(
                    Constants.GAME_WIDTH - MARKER_OFFSET,
            ...
            if (intersect != null) {
                mrc.setVisible(true);
                VectorD diff = parentPos
                    .getPosition()
                    .diff(playerPos.getPosition());
                double rot = Math.atan2(diff.getValue(0), diff.getValue(1));
                TransformationStorage mpc = (TransformationStorage) entity.getComponent(
                    SpaxelComponent.TRANSFORMATION
                );
    ...
    private static VectorD getIntersection(
        VectorD leftTop,
        VectorD rightBot,
        VectorD playerPos,
        VectorD enemyPos
