...
public final class ElementCreator {
    ...
    public static Element createItemView(Entity item) {
        RenderableStorage sc = (RenderableStorage) item.getComponent(
            SpaxelComponent.RENDERABLE
        );
        CooldownStorage cc = (CooldownStorage) item.getComponent(
            SpaxelComponent.COOLDOWN
        );
        StackStorage stc = (StackStorage) item.getComponent(
        ...
        cooldown
            ...
            .setProperty(
                "completion",
                String.valueOf(
                    (double) cc.getCurrentCooldown() / cc.getMaxCooldown()
                )
            );
